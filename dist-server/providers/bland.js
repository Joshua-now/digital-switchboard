...
export async function createCall(leadId, clientId, phone, instructions, transferNumber) {
    ...
    try {
        ...
        if (response.ok && data.call_id) {
            ...
            await createAuditLog('CALL_INITIATED', `Call initiated to ${phone}`, clientId, { leadId, callId: call.id, providerCallId: data.call_id });
            ...
            await createAuditLog('CALL_FAILED', `Call failed: ${data.error || data.message}`, clientId, { leadId, callId: call.id, error: data });
            return {
                success: false,
        ...
        await createAuditLog('CALL_ERROR', `Call error: ${error.message}`, clientId, { leadId, phone, error: error.message });
