...
import { requireAuth, requireSuperAdmin, agencyScope, verifyPassword, hashPassword, generateToken, } from '../middleware/auth.js';
...
async function saveRoutingConfig(req, res) {
    try {
        const scope = agencyScope(req.user);
        const clientExists = await prisma.client.findFirst({
            where: { id: req.params.id, ...scope },
...
router.get('/leads', requireAuth, async (req, res) => {
    try {
        ...
        if (status)
            where.callStatus = String(status);
        if (search) {
...
router.post('/admin/migrate-clients', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
        ...
        if (!agencyName) {
            res.status(400).json({ error: 'agencyName required' });
            return;
        }
        const agency = await prisma.agency.findFirst({ where: { name: agencyName } });
        if (!agency) {
            res.status(404).json({ error: `Agency "${agencyName}" not found` });
            return;
        }
...
router.patch('/admin/agencies/:id', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
